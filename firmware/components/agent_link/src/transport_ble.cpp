...
//    GATT Service B 0xFFB0: 0xFFB1 file/OTA command channel (WRITE + NOTIFY). Same frame
//     format and same core dispatcher as 0xFFC1 - it exists because the official App writes
//     0x37/0x56 (OTA) there; responses to those command ids are notified back on 0xFFB1.
// Data plane:
...
//    bulk audio uplink (device -> App): L2CAP CoC (PSM 0x0081) send + control events 0x52/0x53.
//    Image snapshot uplink (device -> App): L2CAP CoC (PSM 0x0082) send + control events 0x54/0x55.
//    OTA firmware downlink (App -> device): the same L2CAP CoC (PSM 0x0081); while an upgrade is
//     receiving, incoming bytes go to the OTA engine instead of the TTS path, and peer credit is
//     withheld whenever its buffer runs low (see the RX gate below).
...
// ── Service B (file transfer / OTA commands) ──
// Kept for App compatibility: the official App writes the OTA commands here, not on 0xFFC1.
constexpr uint16_t kSvcFile       = 0xFFB0;
constexpr uint16_t kChrFileCmd    = 0xFFB1;  // WRITE receives 0x37/0x56 + NOTIFY returns their responses
...
const ble_uuid16_t s_uuid_file_svc  = BLE_UUID16_INIT(kSvcFile);
const ble_uuid16_t s_uuid_file_cmd  = BLE_UUID16_INIT(kChrFileCmd);
...
uint16_t s_h_file  = 0; // 0xFFB1 (file / OTA command channel)
...
// A separate PSM from audio so image snapshots and audio can stream concurrently without interleaving on a single byte channel.
...
int GattAccess(uint16_t /*conn*/, uint16_t /*attr*/, struct ble_gatt_access_ctxt* ctxt, void* /*arg*/) {
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR: {
        // Command channels 0xFFC1 (control) and 0xFFB1 (file/OTA) both take writes; they carry the
        // same frame format, so both go to the one core dispatcher, which routes by command_id.
        if ((chr == kChrCmdRequest || chr == kChrFileCmd) && ctxt->om) {
            uint8_t buf[512];
            uint16_t n = 0;
...
// Service B (file transfer / OTA): 0xFFB1 FILE_CMD (WRITE + NOTIFY).
const struct ble_gatt_chr_def s_chrs_file[] = {
    { .uuid = &s_uuid_file_cmd.u, .access_cb = GattAccess,
      .flags = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_NOTIFY, .val_handle = &s_h_file },
    { 0 },
};
...
const struct ble_gatt_svc_def s_svcs[] = {
    ...
    { .type = BLE_GATT_SVC_TYPE_PRIMARY, .uuid = &s_uuid_file_svc.u,  .characteristics = s_chrs_file },
...
// ── bulk audio uplink (L2CAP CoC data plane + control events 0x52/0x53) ──
...
AudioStreamState s_rec;
uint32_t       s_rec_counter = 0;

// Image snapshot uplink (L2CAP CoC PSM 0x0082 + control events 0x54/0x55)
// its own PSM and state so it runs concurrently with the audio stream. One image at a time.
...
// RX gate (OTA back-pressure). Rearming is what grants the peer fresh credit, so skipping it
// is how the device tells the App "stop sending" without dropping a byte. Set when the OTA
// buffer ran low; cleared by ResumeL2capRx() once the worker has made room.
std::atomic<bool> s_l2cap_rearm_pending{false};

// Called from the OTA worker after it consumes a chunk (and periodically as a backstop, since
// a rearm can also fail transiently on an empty mbuf pool with nobody else to retry it).
void ResumeL2capRx() {
    if (!s_l2cap_rearm_pending.load(std::memory_order_acquire)) return;  // hot path: one atomic load
    struct ble_l2cap_chan* chan = s_l2cap_chan;                          // snapshot: the channel can drop
    if (!s_l2cap_connected || !chan) return;   // reconnect re-arms the first buffer on COC_ACCEPT
    // Claim the flag: only whoever flips it true->false does the rearm, so the host task and the
    // worker can never double-grant. While the gate is shut the peer has no credit and cannot
    // deliver more data, so in practice they do not race at all.
    if (s_l2cap_rearm_pending.exchange(false, std::memory_order_acq_rel)) {
        if (!RearmL2capRx(chan)) s_l2cap_rearm_pending.store(true, std::memory_order_release);
    }
}
...
int L2capEvent(struct ble_l2cap_event* event, void* arg) {
    ...
    case BLE_L2CAP_EVENT_COC_DISCONNECTED:
        if (is_img) {
            ...
            s_l2cap_stalled.store(false, std::memory_order_release); // unblock a stalled audio worker so it can exit
            ...
            s_l2cap_rearm_pending.store(false, std::memory_order_release);
            // The firmware arrives on this channel: losing it mid-upgrade means the rest is never
            // coming, so fail now (0x38 phase=Failed error=2035) instead of waiting out the timeout.
            agentlink::ota::Abort("L2CAP disconnected");
        }
        ESP_LOGI(TAG, "L2CAP CoC disconnected (PSM 0x%04X)", is_img ? kImgPsm : kL2capPsm);
        return 0;
    case BLE_L2CAP_EVENT_COC_DATA_RECEIVED: {
        struct os_mbuf* sdu = event->receive.sdu_rx;
        // The image channel is uplink-only,ignore any downlink bytes on it. The audio channel carries
        // TTS voice downlink, or the new firmware while an OTA is running. Never block here (host task).
        if (!is_img) {
            const bool to_ota = agentlink::ota::IsBusy();
            ...
            for (uint16_t off = 0; off < total; ) {
                ...
                if (os_mbuf_copydata(sdu, off, n, buf) == 0) {
                    if (to_ota)             agentlink::ota::FeedData(buf, n);
                    else if (s_on_stream)   s_on_stream(AGENT_STREAM_VOICE, buf, n);
                }
                off += n;
            }
        }
        os_mbuf_free_chain(sdu);
        // Replenishing the rx buffer is also what grants the peer credit. On the audio channel,
        // hold it back while the OTA buffer is nearly full so the App pauses instead of
        // overrunning us; the OTA worker calls ResumeL2capRx() once it has written the pending
        // bytes to flash. The gate (and its pending flag) belong to this channel alone - the
        // image channel is never the one being throttled, so it always rearms immediately.
        if (is_img || agentlink::ota::WantsMore()) {
            RearmL2capRx(event->receive.chan);
        } else {
            s_l2cap_rearm_pending.store(true, std::memory_order_release);
        }
        return 0;
    }
    case BLE_L2CAP_EVENT_COC_TX_UNSTALLED:
        if (is_img) s_img_stalled.store(false, std::memory_order_release);   // tx credits restored
        else        s_l2cap_stalled.store(false, std::memory_order_release); // audio worker may send again
        return 0;
    default:
        return 0;
    }
...
esp_err_t StartL2capServer() {
    // Audio channel (PSM 0x0081): downlink playback + uplink bulk audio
    ...
    ESP_LOGI(TAG, "L2CAP CoC server on PSM 0x%04X (MTU=%d MPS=%d) — App opens this for audio",
...
// bulk audio uplink implementation
...
void AudioStreamTask(void*) {
    ESP_LOGI(TAG, "audio stream worker started (transfer_id=%u)", static_cast<unsigned>(s_rec.transfer_id));
    ...
    ESP_LOGI(TAG, "audio stream %u ended (status=%u valid_bytes=%u)",
...
esp_err_t AudioStreamStart(const char* name, size_t name_len) {
    ...
    if (!s_l2cap_connected) {  // "保障通道": the App must have opened the L2CAP CoC before we can push
        ESP_LOGW(TAG, "audio stream: L2CAP CoC not open — App must connect PSM 0x%04X first", kL2capPsm);
    ...
    if (!RecSendStreamStart(s_rec.transfer_id, name, name_len)) {
        ESP_LOGE(TAG, "audio stream: 0x52 StreamStart notify failed");
        return ESP_FAIL;
    }
    s_rec.active.store(true, std::memory_order_release);
    if (xTaskCreate(AudioStreamTask, "agentlink_audio", 6144, nullptr, 5, &s_rec.task) != pdPASS) {
        s_rec.active.store(false, std::memory_order_release);
        ESP_LOGE(TAG, "audio stream: task create failed");
        return ESP_ERR_NO_MEM;
    }
    ESP_LOGI(TAG, "audio stream %u started (%u-byte name)",
             static_cast<unsigned>(s_rec.transfer_id), static_cast<unsigned>(name_len));
    return ESP_OK;
}

esp_err_t AudioStreamEnqueue(const uint8_t* data, size_t len) {
    ...
    if (s_rec.queued_bytes.load(std::memory_order_acquire) + len > kRecQueueHardCap) {
        ...
        ESP_LOGW(TAG, "audio queue cap (%uKB) — truncating stream %u",
...
esp_err_t AudioStreamEnd(bool complete, const uint8_t* final_header, size_t hdr_len) {
...
// Mirrors the audio path; reuses kRecMaxSdu / kRecStallWaitMs / kRecStartDelayMs (generic L2CAP params).
...
void ImageTask(void*) {
    ...
    // Head start for 0x54 ImageStart (same race as the audio stream 0x52): let the App register the session before bytes arrive.
    vTaskDelay(pdMS_TO_TICKS(kRecStartDelayMs));
    while (true) {
...
// ── Link speed: connection interval -> 2M PHY -> DLE, one step per completion event
// The App streams ~2MB OTA images and minute-long audio over this link, so the default
// parameters a phone picks are not good enough.
void RequestFastConnParams(uint16_t conn_handle) {
    ...
    if (rc != 0) ESP_LOGW(TAG, "conn param update request failed: %d", rc);
    else         ESP_LOGI(TAG, "requested fast conn params (7.5-15ms)");
}

void RequestTwoMPhy(uint16_t conn_handle) {
    const int rc = ble_gap_set_prefered_le_phy(conn_handle,
                                               BLE_GAP_LE_PHY_2M_MASK,
                                               BLE_GAP_LE_PHY_2M_MASK,
                                               BLE_GAP_LE_PHY_CODED_ANY);
    if (rc != 0) ESP_LOGW(TAG, "2M PHY request failed: %d", rc);
    else         ESP_LOGI(TAG, "requested 2M PHY");
}

void RequestDataLen(uint16_t conn_handle) {
    const int rc = ble_gap_set_data_len(conn_handle, 251, 0x0848);
    if (rc != 0) ESP_LOGW(TAG, "DLE request failed: %d", rc);
    else         ESP_LOGI(TAG, "requested max data length (251 octets)");
}

bool s_phy_requested = false;   // one shot per connection, both cleared on disconnect
bool s_dle_requested = false;
...
int GapEvent(struct ble_gap_event* event, void* /*arg*/) {
    ...
    case BLE_GAP_EVENT_CONNECT:
        if (event->connect.status == 0) {
            ...
            // Kick off the link-speed chain. Only the first step goes out here - the rest are
            // fired from their completion events (see RequestFastConnParams).
            RequestFastConnParams(s_conn_handle);
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        ...
        s_phy_requested = false; s_dle_requested = false;   // renegotiate on the next connection
        ...
        s_rec.truncated.store(true, std::memory_order_release);  // and the audio worker
        s_rec.end_req.store(true, std::memory_order_release);
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        // Connection interval settled -> next step: 2M PHY (see RequestFastConnParams).
        if (!s_phy_requested && s_conn_handle != BLE_HS_CONN_HANDLE_NONE) {
            s_phy_requested = true;
            RequestTwoMPhy(s_conn_handle);
        }
        return 0;
    case BLE_GAP_EVENT_PHY_UPDATE_COMPLETE:
        // PHY settled -> last step: data length extension.
        if (!s_dle_requested && s_conn_handle != BLE_HS_CONN_HANDLE_NONE) {
            s_dle_requested = true;
            RequestDataLen(s_conn_handle);
        }
        return 0;
...
esp_err_t ble_start(void* /*impl*/) {
    ...
    // Let the OTA worker lift the RX gate it caused (see ResumeL2capRx).
    agentlink::ota::SetResumeRxHook(ResumeL2capRx);

    ble_store_config_init();  // install the key store
    nimble_port_freertos_init(HostTask);
    ESP_LOGI(TAG, "BLE started — Services 0xFFA0/0xFFB0/0xFFC0 registered; advertising on sync");
    return ESP_OK;
}
...
// Control-plane send: route by frame message_type. Event (0x03) -> 0xFFC4; response (0x02) ->
// 0xFFC1, except for the file/OTA command ids, whose responses go back on 0xFFB1 where the App
// wrote them. Routing on the command id rather than remembering the write's characteristic keeps
// this stateless and matches the App's own channel split.
bool IsFileChannelCmd(uint8_t command_id) {
    switch (command_id) {
    case 0x06:  // start file download
    case 0x08:  // start WiFi file server
    case 0x37:  // StartOtaUpgrade
    case 0x55:  // abort file download
    case 0x56:  // AbortOtaUpgrade
        return true;
    default:
        return false;
    }
...
esp_err_t ble_send_ctrl(void* /*impl*/, const uint8_t* frame, size_t len) {
    if (!frame || len < 3) return ESP_ERR_INVALID_ARG;
    const uint8_t msg_type   = frame[1] & 0x7F;
    const uint8_t command_id = frame[2];
    uint16_t handle;
    if (msg_type == 0x03 /*Event*/)             handle = s_h_evt;
    else if (IsFileChannelCmd(command_id))      handle = s_h_file;
    else                                        handle = s_h_cmd;
    return Notify(handle, frame, len);
}

// Data plane: VOICE over GATT Notify 0xFFA1, AUDIO and IMAGE over L2CAP CoC. FILE and VIDEO are
// declared in agent_stream_t but this backend does not carry them yet.
esp_err_t ble_stream_start(void* /*impl*/, agent_stream_t type, const uint8_t* meta, size_t meta_len) {
    ...
    if (type == AGENT_STREAM_AUDIO)                          // bulk audio: 0x52 + L2CAP uplink
        return AudioStreamStart(reinterpret_cast<const char*>(meta), meta_len);
...
esp_err_t ble_send_stream(void* /*impl*/, agent_stream_t type, const uint8_t* data, size_t len) {
    ...
    if (type == AGENT_STREAM_AUDIO) return AudioStreamEnqueue(data, len);
...
esp_err_t ble_stream_end(void* /*impl*/, agent_stream_t type, bool complete, const uint8_t* meta, size_t meta_len) {
    ...
    if (type == AGENT_STREAM_AUDIO) return AudioStreamEnd(complete, meta, meta_len);  // 0x53 (meta = final_header)
...
// Is the App's L2CAP data channel (PSM 0x0081) open? The core requires it before accepting an
// OTA: the firmware bytes have nowhere else to arrive.
extern "C" bool agent_transport_ble_l2cap_ready(void) { return s_connected && s_l2cap_connected; }

// Own BLE address, MSB-first (the order the App sees in the advertisement). NimBLE keeps it
