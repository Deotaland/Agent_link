...
- `agent_link_register_io` / `agent_link_push_reading`：声明传感器与执行器并上报读数，执行器命令会回到你注册的回调。
...
└── components/
    ...
    └── esp_lcd_sh8501/          SH8501 完整 vendor 初始化，供 OTP 未烧录的屏使用
```
...
| 语音下行：App 经 L2CAP CoC `0x0081` 推音频 —— raw PCM 或 IMA-ADPCM，解码成 PCM16 后交给 `on_audio_out`，并用 `0x20` 流控保证长语音不会跑在喇叭前面 | 已实现，尚未真机验证 |
| 通用 I/O：`register_io`、`push_reading`、执行器下行（manifest `0x18`、读数 `0x19`、执行 `0x33`、拉取 `0x34`） | 已实现，尚未真机验证 |
| OTA 固件升级：App 经 L2CAP `0x0081` 推镜像（`0x37`/`0x56`，进度 `0x38`），SHA-256 校验，带回滚保护 | 已实现，尚未真机验证 |
| 录音 / 文件下载走 L2CAP 上行 | 未实现 |
...
| 板子 `work-badge` | 跑在量产板上的电子工牌（量产引脚表、厂家简码 SH8501 屏）：LVGL 名片屏，内容由 App 经通用 I/O 下发并存入 NVS，另有 OTA 进度屏。生成 CJK 字库之前只能显示拉丁字符（[README](boards/work-badge/README.md)） |
...
- 服务 `0xFFB0`（文件/OTA）：`0xFFB1` 命令（App 写 `0x37`/`0x56`，设备通知回响应）。保留它是为了让线上 App 的 OTA 流程不用改。
- L2CAP CoC，PSM `0x0081`：接收下行 TTS 音频并转给 `on_audio_out`，OTA 期间承载固件镜像。同一通道上的录音、文件上行尚未实现。

通用 I/O 走控制面：自描述清单（事件 `0x18`）、带类型的读数（事件 `0x19`）、执行器命令（`0x33`）、清单拉取（`0x34`）。语音上行帧是 6 字节头后接 `session_id`（4）、`sequence`（4）、`flags`（1）和 PCM；整帧控制在 220 字节以内，每次通知的 PCM 取 `min(MTU - 3 - 15, 205)` 并向下取偶数。
...
板级目录布局参考了 [xiaozhi-esp32](https://github.com/78/xiaozhi-esp32)（同为 MIT）。

`components/esp_lcd_sh8501/` 是乐鑫的 SH8501 屏驱动，`components/esp_lcd_sh8501_lk/` 是它针对厂家简码屏的
修改版；二者连同 `managed_components/` 下的全部组件均为 Apache-2.0，各自保留版权与修改声明。
