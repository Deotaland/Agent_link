...
public:
    ...
    // Blit a rectangle of big-endian RGB565 pixels (row-major, w*h*2 bytes).
    //
    // The source may live anywhere, PSRAM included: pixels are copied through the driver's
    // internal DMA staging buffer a few rows at a time, which also keeps every SPI transaction
    // within the bus's max_transfer_sz. Blocks until the last row is on the wire, so the caller
    // may reuse or free the source as soon as it returns.
...
private:
    ...
    // Allocate/grow the internal DMA staging buffer to at least `bytes`.
