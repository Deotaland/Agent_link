...
esp_err_t Sh8501Panel::Init(const Sh8501Config& cfg) {
    ...
    // ── panel (SH8501): init_cmds=NULL -> the component's built-in full sequence
...
esp_err_t Sh8501Panel::EnsureStripe(size_t bytes) {
...
esp_err_t Sh8501Panel::DrawBitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
    ...
    // Stage through the same internal DMA buffer FillSolid uses, so an arbitrary (PSRAM) source
    // works and no single transaction can exceed the SPI bus limit set up in Init().
    ...
    for (uint16_t row = 0; row < h; row += static_cast<uint16_t>(rows_per_chunk)) {
        ...
        ESP_RETURN_ON_ERROR(
            ...
            TAG, "draw");
        // Single staging buffer: the next chunk overwrites it, so this DMA must land first.
        if (!WaitDone(expect, 500)) {
            ESP_LOGE(TAG, "blit tx timeout row=%u", static_cast<unsigned>(y + row));
            return ESP_ERR_TIMEOUT;
        }
    }
    return ESP_OK;
